...
describe("PATCH /api/v1/users/[username]", () => {
  ...
  describe("Default user", () => {
    ...
    test("With duplicated 'email'", async () => {
      ...
      const email2 = await orchestrator.createUser({
        email: "email2@email.com",
      });

      const activatedUser2 = await orchestrator.activateUser(email2);
      const sessionObject2 = await orchestrator.createSession(
        activatedUser2.id,
      );
    ...
    test("With new 'password'", async () => {
      ...
      const activatedUser = await orchestrator.activateUser(uniquePassword);
      const sessionObject = await orchestrator.createSession(activatedUser.id);

      const response = await fetch(
        `http://localhost:3000/api/v1/users/${uniquePassword.username}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  describe("Privileged user", () => {
    test("With `update:others` targeting `defaultUser`", async () => {
      ...
      const responseBody = await userBUpdate.json();
